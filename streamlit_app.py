...
def fetch_all_quotes(symbols: tuple[tuple[str, str], ...]) -> dict[str, Any]:
    ...
    for code, exchange in symbols:
        ...
        code_to_exchange[code] = exchange

    # 2. 第一波：單次網路請求大規模批量下載（period="5d" 可完美覆蓋跨週末與休市）
    try:
        df = yf.download(primary_tickers, period="5d", group_by="ticker", progress=False)
    except Exception as e:
        errors.append(f"第一批次多股批量下載失敗: {e}")
        df = pd.DataFrame()

    # 3. 向量解析第一波結果，並過濾出未成功捕捉的股票代碼
    missing_codes = []
    ...
    for ticker in primary_tickers:
        ...
        if is_multi and (ticker, "Close") in df.columns:
            ...
            if not close_series.empty:
                try:
                    ...
                    if (ticker, "Volume") in df.columns:
                        vol_series = df[(ticker, "Volume")].dropna()
                    ...
                    if len(close_series) >= 2:
                        ...
                        chg_pct = ((price / prev_close) - 1.0) * 100.0 if prev_close else None
                    else:
                        chg_pct = None
                    ...
                    errors.append(f"解析 {code} 數據錯誤: {ex}")

        if not parsed:
            missing_codes.append(code)

    # 4. 第二波：針對沒抓到的代碼進行反向市場後綴備用下載 (.TW <-> .TWO)
    if missing_codes:
        fallback_tickers = []
        fallback_ticker_to_code = {}
        for code in missing_codes:
            exchange = code_to_exchange[code]
            suffix = ".TW" if exchange == "otc" else ".TWO"
            ticker = f"{code}{suffix}"
            fallback_tickers.append(ticker)
            fallback_ticker_to_code[ticker] = code

        try:
            df_fb = yf.download(fallback_tickers, period="5d", group_by="ticker", progress=False)
        except Exception as e:
            errors.append(f"第二批次備用批量下載失敗: {e}")
            df_fb = pd.DataFrame()

        is_multi_fb = isinstance(df_fb.columns, pd.MultiIndex) if not df_fb.empty else False

        for ticker in fallback_tickers:
            code = fallback_ticker_to_code[ticker]
            if is_multi_fb and (ticker, "Close") in df_fb.columns:
                close_series = df_fb[(ticker, "Close")].dropna()
                if not close_series.empty:
                    try:
                        price = float(close_series.iloc[-1])
                        
                        if (ticker, "Volume") in df_fb.columns:
                            vol_series = df_fb[(ticker, "Volume")].dropna()
                            vol = float(vol_series.iloc[-1]) if not vol_series.empty else 0.0
                        else:
                            vol = 0.0
                        vol_lots = vol / 1000.0
                        ...
                        errors.append(f"解析備用 {code} 數據錯誤: {ex}")

    # 5. 回填並對齊看板全局統計時間戳
    for q in quotes.values():
        if q.get("time") and q["time"] != "--:--:--":
            latest_time = q["time"]
        if q.get("date") and q["date"] != "--":
            latest_date = q["date"]
